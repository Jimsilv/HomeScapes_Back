...
class TransactionAdmin(admin.ModelAdmin):
    ...
    def approve_withdrawals(self, request, queryset):
        """Admin action to approve withdrawals and auto-deduct balance."""
        for transaction in queryset.filter(transaction_type="withdrawal", status="pending"):
            user_profile = transaction.user.profile  # Access the UserProfile model

            if user_profile.balance >= transaction.amount:
                with db_transaction.atomic():  # Ensuring atomicity
